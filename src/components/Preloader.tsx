...
export default function Preloader({ setComplete }: PreloaderProps) {
    ...
    return (
        <div ref={containerRef} className="fixed inset-0 z-99 flex flex-col overflow-hidden">
            {/* Brand Name */}
            <h1 aria-label="Lawal, written backwards" className="title text-accent text-9xl z-99999 flex-center h-1/2 mix-blend-difference">
            ...
            <div className="absolute inset-x-0 bottom-1/12 text-accent mix-blend-difference flex items-center justify-center z-50 px-10">
                ...
                <svg ref={logoRef} width="60" height="60" viewBox="0 0 100 100" fill="none">
                ...
                <span
                    ref={sloganRef}
                    className="header text-center max-w-4xl"
                >
                ...
                {[...Array(columnCount)].map((_, i) => (
                    <div key={i} className="column-wrapper relative h-1/2 flex-1 overflow-hidden">
                        <div className="absolute inset-x-0 top-0 h-15 flex items-center justify-center gap-1">
                            <div className={`line-${i} progress-line h-px bg-primary w-full`}></div>
                            <span className={`text-${i} progress-text font-mono text-sm font-bold text-primary opacity-0`}>
                                {progressSteps[i] ?? currentProgress}%
                            </span>
                        </div>
                        <div className={`shutter shutter-${i} absolute inset-0 bg-primary z-10`} />
                    </div>
                ))}
            </div>
        </div>
