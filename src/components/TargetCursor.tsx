...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    if (containerRef) {
      ...
      if (container) {
        container.addEventListener("mouseenter", setCursorNone);
        container.addEventListener("mouseleave", resetCursor);
        // Set initial opacity to 0 if we have a container
        if (cursorRef.current) cursorRef.current.style.opacity = "0";
      }
    } else {
    ...
    return () => {
      ...
      if (containerRef?.current) {
        containerRef.current.removeEventListener("mouseenter", setCursorNone);
        containerRef.current.removeEventListener("mouseleave", resetCursor);
      }
      spinTl.current?.kill();
      document.body.style.cursor = originalCursor;
      isActiveRef.current = false;
